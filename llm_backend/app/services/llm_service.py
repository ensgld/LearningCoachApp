...
async def preload_models():
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(OLLAMA_GENERATE_URL, json=payload, timeout=REQUEST_TIMEOUT)
...
async def ask_llama(user_message: str, history: list[dict] = [], stream: bool = False) -> str | AsyncGenerator:
    ...
    if stream:
        async def generate():
            async with httpx.AsyncClient() as client:
                async with client.stream("POST", OLLAMA_URL, json=payload, timeout=REQUEST_TIMEOUT) as response:
                    response.raise_for_status()
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(OLLAMA_URL, json=payload, timeout=REQUEST_TIMEOUT)
            response.raise_for_status()
